...
imageDir = fullfile("Zhu (Roboflow)", "images");
...
annoDir = fullfile("Zhu (Roboflow)", "annotations");
...
if ~exist('detection_figures_zhu_roboflow', 'dir')
    mkdir('detection_figures_zhu_roboflow');
end
...
for i = 1:size(detectionResults, 1)
    ...
    fig_filename = fullfile('detection_figures_zhu_roboflow', [filename, '.fig']);
    savefig(ax, fig_filename);
    
    % Save the figure as .m (using saveas)
    m_filename = fullfile('detection_figures_zhu_roboflow', [filename, '.m']);
    saveas(ax, m_filename);
    
    % Save the figure as .png
    png_filename = fullfile('detection_figures_zhu_roboflow', [filename, '.png']);
...
save('zhu_roboflow_metrics.mat', 'metrics');
