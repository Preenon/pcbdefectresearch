...
imageDir = fullfile("VOC_PCB","JPEGImages");
...
annoDir = fullfile("VOC_PCB","Annotations");
...
writetable(countEachLabelTable, "countEachLabelTable(TDD).csv");
...
writetable(averagePrecisionTable, "AveragePrecisionTable(TDD).csv");
...
for class = 1:6
    ...
    grid on
    ax = gca;
    saveas(ax, "PR Curve for " + classNames(class) + "(TDD).m");
    exportgraphics(ax, "PR Curve for " + classNames(class) + "(TDD).jpg");
...
ax = gca;
saveas(ax, "Bounding Box Area Distribution(TDD).m");
exportgraphics(ax, "Bounding Box Area Distribution(TDD).jpg");
...
writetable(metricsByAreaTable, "metricsByAreaTable(TDD).csv");